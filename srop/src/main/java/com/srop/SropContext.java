...
import com.srop.handler.ServiceMethodDefinition;
 ...
 */
public class SropContext {
    protected Logger logger = LoggerFactory.getLogger(getClass());

    private Map<String, ServiceMethodHandler> serviceHandlerMap = new HashMap<String, ServiceMethodHandler>();

    public SropContext(final ApplicationContext context) {
        if (logger.isDebugEnabled()) {
            logger.debug("对Spring上下文中的Bean进行扫描，查找srop服务方法: " + context);
        }
                ...
                ReflectionUtils.doWithMethods(handlerType, new ReflectionUtils.MethodCallback() {
                            public void doWith(Method method) throws IllegalArgumentException, IllegalAccessException {
                                ...
                                if (method.getParameterTypes().length > 1) {//handler method's parameter
                                    throw new SropException(method.getDeclaringClass().getName() + "." + method.getName()
                                            + "的入参只能是" + SropRequest.class.getName() + "或无入参。");
                                } else if (method.getParameterTypes().length == 1) {
                                    Class<?> paramType = method.getParameterTypes()[0];
                                    if (!ClassUtils.isAssignable(SropRequest.class, paramType)) {
                                        throw new SropException(method.getDeclaringClass().getName() + "." + method.getName()
                                                + "的入参必须是" + SropRequest.class.getName());
                                    }
                                    boolean ropRequestImplType = !(paramType.isAssignableFrom(SropRequest.class) ||
                                            paramType.isAssignableFrom(SropRequest.class));
                                    serviceMethodHandler.setSropRequestImplType(ropRequestImplType);
                                    serviceMethodHandler.setRequestType((Class<? extends SropRequest>) paramType);
                                } else {
